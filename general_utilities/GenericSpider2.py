...
class GenericSpider(scrapy.Spider):
    ...
    def __init__(self, start_url, depth, save_file_path, stay_within, azure_creds=None, azure_container=None, *args, **kwargs):
        super(GenericSpider, self).__init__(*args, **kwargs)
        self.start_urls = [start_url]
        self.depth = int(depth)
        self.save_file_path = save_file_path
        self.stay_within = stay_within
        self.seed_url = start_url
    ...
    def parse(self, response):
        ...
        page_path = urlparse(response.url).path.strip('/').replace('/', '_')
        if self.azure_creds and self.azure_container:
            blob_name = f"{self.save_file_path}/{url_domain}/{page_path}.json"
            self.lzdev(self.azure_container, blob_name, page_data)
        else:
            if not os.path.exists(os.path.join(self.save_file_path, url_domain)):
                os.makedirs(os.path.join(self.save_file_path, url_domain))
            page_filename = os.path.join(self.save_file_path, f"{url_domain}/{page_path}.json")
    ...
    def lzdev(self, container_name, blob_name, data):
        try:
            blob_service_client = BlobServiceClient.from_connection_string(self.azure_creds)
...
def run_spider(start_url, depth=1, stay_within=True, save_file_path='/path/to/local/dir', azure_creds=None, azure_container=None):
    domain = urlparse(start_url).netloc
    if not save_file_path.endswith('/'):
        save_file_path += '/'
    process = CrawlerProcess(settings={
        'DEPTH_LIMIT': int(depth),
        'LOG_ENABLED': False
    })
    process.crawl(GenericSpider, start_url=start_url, depth=depth, save_file_path=save_file_path, stay_within=stay_within, azure_creds=azure_creds, azure_container=azure_container)
    process.start()
    print(f"Information being written to: {save_file_path + domain if not azure_creds else 'Azure Blob Storage'}")
...
if __name__ == "__main__":
    ...
    depth = 2
    stay_within = True
    run_spider(start_url, depth, stay_within, save_file_path='your/local/path', azure_creds='your_azure_connection_string', azure_container='your_container_name')
